...
```
# Create a tooltip text for each Builder 
```{r echo=FALSE}

tooltip_text <- mapply(function(builder,projects) {
  paste("Builder:", builder, "<br>", 
        "Projects:", paste(projects, collapse = ", "))
}, dfChart1data$buildeeer_3letter, dfChart1data$Projects, SIMPLIFY = FALSE)
```
...
flextable(data = dfEHFXTotals,
          ...
          theme_vanilla() %>%         
          autofit()
          compose(j = "builder_3letter", value = as_paragraph(tooltip_text) %>%
                    as_html(), .after = TRUE) %>%
