...
def get_projects(builder, status):
    ...
    return dfChart1Data[(dfChart1Data['builder_3letter'] == builder) & (dfChart1Data['closed'] == status)]['proj_name'].tolist()


# Function to generate expandable section content for a builder
def generate_expandable_section(builder_name, projects):
    section_content = html.Ul([html.Li(project) for project in projects])
    return section_content

# Generate HTML table with expandable sections for each builder
def generate_table():
    table_content = []
    for index, row in df_selected.iterrows():
        ...
        closed = row['Closed']
        active = row['Active']
        projects = get_projects(builder_name, 'Closed')
        expandable_section = generate_expandable_section(builder_name, projects)
        row_content = html.Tr([
            html.Td(builder_name),
            html.Td(originations),
            html.Td(closed),
            html.Td(active),
            html.Td(expandable_section)
        ])
...
app.layout = html.Div([
    # Custom header with parameter values
    html.H1("Portfolio Analysis for {params['pFundID']} as of -- {params['pQtrEnd']}"),
    
    # HTML table
    html.Table([
        html.Thead(html.Tr([
            html.Th('Builder'),
            html.Th('Originations'),
            html.Th('Closed'),
            html.Th('Active'),
            html.Th('Projects')
        ])),
        html.Tbody(generate_table())
    ],
    style={
        'font-family': 'Arial, sans-serif',
        'border-collapse': 'collapse',
...
# Set custom index string to include the desired header and placeholders
app.index_string = f"""
<!DOCTYPE html>
<html>
<head>
    <title>Portfolio Analysis</title>
    <!-- Required Dash placeholders -->
    {{%app_entry%}}
    {{%config%}}
    {{%scripts%}}
    {{%renderer%}}
...
<body>
    <!-- Custom header -->
    <h1>Portfolio Analysis for {{params['pFundID']}} as of -- {{params['pQtrEnd']}}</h1>
    <!-- HTML table -->
...
# Write the styled HTML content with JavaScript to the file
with open(output_filepath, "w") as html_file:
    html_file.write(str(app.layout))


'''
## Origination Summary (Number of Investments) 
table_title = "Origination Summary (Number of Investments)"

# Convert the selected DataFrame to an HTML table with the specified CSS classes
styled_html1 = f"""
<html>
<head>
