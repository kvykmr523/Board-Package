...
column_labels = df_selected.columns.tolist()
row_labels = df_selected.index.tolist()

# Specify colors for cells
num_colors = len(data)
cell_colors = [['paleturquoise' if i % 2 == 0 else 'lavender' for _ in range(len(column_labels))] for i in range(num_colors)]
cell_text_colors = [[mcolors.to_rgba('black', alpha=1.0) for _ in range(len(column_labels))] for _ in range(num_colors)]

table_title = "Origination Summary (Number of Investments)"

# Generate the table
generate_table(ax, data, column_labels, row_labels, cell_colors, cell_text_colors)
...
<body>
    ...
    {closed_projects_tables}
    {active_projects_tables}
</body>
</html>
""".format(
    ...
    table_rows='\n'.join([
        '<tr><td>{}</td><td>{}</td><td onclick="toggleClosedProjectsTable(\'{}\')" style="cursor: pointer;">{}</td><td onclick="toggleActiveProjectsTable(\'{}\')" style="cursor: pointer;">{}</td></tr>'.format(row[0], row[1], index, row[2], index, row[3]) for index, row in enumerate(data)]),
    closed_projects_tables='\n'.join(['<table id="closed-projects-{}" style="display: none;"><tr><th colspan="3">Closed Projects for {}</th></tr>{}</table>'.format(index, row[0], ''.join(['<tr><td>{}</td></tr>'.format(project) for project in get_projects(row[0], 'Closed')[1].split('+')])) for index, row in enumerate(data)]),
    active_projects_tables='\n'.join(['<table id="active-projects-{}" style="display: none;"><tr><th colspan="4">Active Projects for {}</th></tr>{}</table>'.format(index, row[0], ''.join(['<tr><td>{}</td></tr>'.format(project) for project in get_projects(row[0], 'Active')[1].split('+')])) for index, row in enumerate(data)])
)
